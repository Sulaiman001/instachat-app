package com.app.instachat.activities.adapters;
...
import com.app.instachat.activities.views.audiowave.AudioPlayerView;
import com.app.instachat.activities.views.voiceplayer.RecordingPlayerView;
    ...
    public void onBindViewHolder(@NonNull final ViewHolder viewHolder, final int position) {
        ...
        if (Utils.isEmpty(attachType)) {
            if (Utils.isEmpty(chat.getType())) {
                ...
                if (chat.getType().equalsIgnoreCase(IConstants.TYPE_IMAGE)) {
            ...
            if (attachType.equalsIgnoreCase(IConstants.TYPE_TEXT)) {
                showTextLayout(viewHolder, chat);
            } else if (attachType.equalsIgnoreCase(IConstants.TYPE_IMAGE)) {
                setImageLayout(viewHolder, chat);
            } else if (attachType.equalsIgnoreCase(IConstants.TYPE_RECORDING)) {
                try {
                    viewHolder.recordingLayout.setVisibility(View.VISIBLE);

                    switch (viewHolder.getItemViewType()) {
                        case MSG_TYPE_LEFT: //OTHER USER's ITEM RECEIVED TO PLAY HERE
                            final String receiverPath = Utils.getReceiveDirectory(mContext, attachType) + IConstants.SLASH + chat.getAttachmentFileName();
                            if (new File(receiverPath).exists()) {
                                viewHolder.recordingPlayerView.setAudio(receiverPath);
                                if (chat.getDownloadProgress() == IConstants.COMPLETED) {
                        ...
                        case MSG_TYPE_RIGHT: //It check logged in user already sent file, then check inside the /.sent/ folder
                            final String path = Utils.getSentDirectory(mContext, attachType) + IConstants.SLASH + chat.getAttachmentFileName();
            ...
            } else if (attachType.equalsIgnoreCase(IConstants.TYPE_AUDIO)) {
                try {
                    ...
                    switch (viewHolder.getItemViewType()) {
                        ...
                        case MSG_TYPE_LEFT:
                            final String receivePath = Utils.getReceiveDirectory(mContext, attachType) + IConstants.SLASH + chat.getAttachmentFileName();
...
//                            Utils.sout("Audio Path right:: " + path + " >>>> " + new File(path).exists());
            ...
            } else if (attachType.equalsIgnoreCase(IConstants.TYPE_DOCUMENT)) {
                try {
            ...
            } else if (attachType.equalsIgnoreCase(IConstants.TYPE_VIDEO)) {
                try {
                    ...
                    switch (viewHolder.getItemViewType()) {
                        case MSG_TYPE_LEFT:
                            try {
                                final File receivePath = new File(Utils.getReceiveDirectory(mContext, attachType) + IConstants.SLASH + chat.getAttachmentFileName());
                    ...
                    viewHolder.imgVideoPlay.setOnClickListener(new SingleClickListener() {
                        ...
                        public void onClickView(View v) {
                            switch (viewHolder.getItemViewType()) {
                                ...
                                case MSG_TYPE_RIGHT:
                                    try {
                                        final File path = new File(Utils.getSentDirectory(mContext, attachType) + IConstants.SLASH + chat.getAttachmentFileName());
            ...
            } else if (attachType.equalsIgnoreCase(IConstants.TYPE_CONTACT)) {
                try {
            ...
            } else if (attachType.equalsIgnoreCase(IConstants.TYPE_LOCATION)) {
                try {
    ...
    public class ViewHolder extends RecyclerView.ViewHolder {
        ...
        private void broadcastDownloadEvent(Chat chat) {
            final Intent intent = new Intent(IConstants.BROADCAST_DOWNLOAD_EVENT);
            intent.putExtra(IConstants.DOWNLOAD_DATA, new DownloadFileEvent(chat, getAbsoluteAdapterPosition()));
            ...
            if (isAudioPlaying) {
                if (Utils.isEmpty(viewHolder.audioPlayerView.getPath())) {
                    ...
                    if (myViewList.isEmpty()) {
                        ...
                        final AudioPlayerView oldPlayerView = myViewList.get(IConstants.ZERO);
                        oldPlayerView.getImgPauseClickListener().onClick(viewHolder.audioPlayerView);
                        myViewList.remove(IConstants.ZERO);
            ...
            if (isRecordingPlaying) {
                if (Utils.isEmpty(viewHolder.recordingPlayerView.getPath())) {
                    ...
                    if (myRecList.isEmpty()) {
                        ...
                        final RecordingPlayerView oldPlayerView = myRecList.get(IConstants.ZERO);
                        oldPlayerView.getImgPauseClickListener().onClick(viewHolder.recordingPlayerView);
                        myRecList.remove(IConstants.ZERO);
        ...
        if (!Utils.isEmpty(myViewList)) {
            try {
                for (int i = 0; i < myViewList.size(); i++) {
                    final AudioPlayerView audioPlayerView = myViewList.get(IConstants.ZERO);
        ...
        if (!Utils.isEmpty(myRecList)) {
            try {
                for (int i = 0; i < myRecList.size(); i++) {
                    final RecordingPlayerView recordingPlayerView = myRecList.get(IConstants.ZERO);
