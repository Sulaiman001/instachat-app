...
public class UserAdapters extends RecyclerView.Adapter<UserAdapters.ViewHolder> implements FastScrollRecyclerView.SectionedAdapter {
    ...
    public void onBindViewHolder(@NonNull ViewHolder viewHolder, int i) {
        ...
        if (user.getIsOnline() == IConstants.STATUS_ONLINE) {
        ...
        viewHolder.itemView.setOnLongClickListener(v -> {
            if (isChat) {
                ...
                Utils.showYesNoDialog(((Activity) mContext), R.string.strDelete, R.string.strDeleteConversion, () -> {
                    Query queryCurrent = FirebaseDatabase.getInstance().getReference().child(IConstants.REF_CHATS).child(currentUser + IConstants.SLASH + receiverId);
                    queryCurrent.getRef().removeValue();
                    Query queryReceiver = FirebaseDatabase.getInstance().getReference().child(IConstants.REF_CHATS).child(receiverId + IConstants.SLASH + currentUser);
    ...
    private void lastMessage(final String userId, final TextView lastMsg, final TextView lastDate, final ImageView imgPath) {
        ...
        try {
            Query reference = FirebaseDatabase.getInstance().getReference(IConstants.REF_CHATS).child(firebaseUser.getUid() + IConstants.SLASH + userId).limitToLast(1);
                ...
                public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                    if (dataSnapshot.hasChildren()) {
                        ...
                        if ("default".equals(theLastMsg)) {
                            ...
                            lastDate.setText(IConstants.EMPTY);
                        } else {
    ...
    private void lastMessageCount(final String userId, final TextView txtUnreadCounter) {
        ...
        try {
            Query reference = FirebaseDatabase.getInstance().getReference(IConstants.REF_CHATS).child(firebaseUser.getUid() + IConstants.SLASH + userId).orderByChild(IConstants.EXTRA_SEEN).equalTo(false);
    ...
    public String getSectionName(final int position) {
        if (!Utils.isEmpty(mUsers)) {
            return mUsers.get(position).getUsername().substring(IConstants.ZERO, IConstants.ONE);
        } else {
            return null;
        }
    }
